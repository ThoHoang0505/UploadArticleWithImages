...
<body class="bg-gray-100 p-6">
    <div class="container mx-auto">
        <div class="flex justify-between items-center mb-8">
            <h1 class="text-4xl font-bold text-gray-800">Danh Sách Bài Viết</h1>
            <div>
                @if(Auth::check())
        ...
        @if($baiviets->isNotEmpty())
            <div class="mb-12">
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-8">
                    @foreach($baiviets->sortByDesc('luot_xem')->take(3) as $baiviet)
                        <a href="{{ route('baiviet.show', ['MaBT' => $baiviet->MaBT]) }}" class="block p-6 rounded-lg shadow-md hover:shadow-lg transition-shadow duration-300 transform hover:-translate-y-1 highlighted-news">
                            <h2 class="text-2xl font-bold mb-3">{{ $baiviet->TieuDeBT }}</h2>
                            ...
                            @if($baiviet->AnhDaiDien)
                                <img src="{{ asset('storage/' . $baiviet->AnhDaiDien) }}" alt="Ảnh đại diện" class="w-full h-48 object-cover mb-4 rounded-lg">
                            @endif
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8">
                @foreach($baiviets as $baiviet)
                    <a href="{{ route('baiviet.show', ['MaBT' => $baiviet->MaBT]) }}" class="block bg-white p-6 rounded-lg shadow-md hover:shadow-lg transition-shadow duration-300 transform hover:-translate-y-1 news-card">
